...
function Editor (props) {
  ...
  const getItem = () => {
    fetch('/api' + props.of + '/details/' + props.id, {
      method: 'GET',
      headers: {
  ...
  const handleSubmit = (item) => {
    ...
    .then((json) => {
      ...
      props.history.push(props.saveRef || props.of);
    })
    .catch((exception) => {
      setLoading(false);
      setError(exception.message);
  ...
  return (
      <Formsy 
        ...
        <div>
          ...
          <Button component={Link} to={ props.saveRef || props.of } className="mx-auto mt-32 mb-80">Cancel</Button>
        </div>
      </Formsy>
  );
}
